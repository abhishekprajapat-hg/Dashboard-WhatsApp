...
**Bug 1 - no AUTHENTICATION template category existed anywhere in the app.** Meta requires OTP/verification-code content to be submitted under the `AUTHENTICATION` category, which has a completely different, fixed component shape (auto-generated body via `add_security_recommendation`, a `FOOTER` with `code_expiration_minutes`, and a `BUTTONS` component with `type: "OTP"`) - not free-form body text. This app only ever supported `MARKETING`/`UTILITY`. A `signup_otp` template submitted as `utility` was confirmed rejected by Meta for exactly this reason. **Fixed**: added `authentication` as a real category end to end - `routes/templates.js`'s local `categories`/`metaCategoryByLocalCategory` lists, a new `buildAuthTemplateComponents()` used by the `/:id/submit` route when the category resolves to `AUTHENTICATION`, and the client's category dropdown (`TemplatesView.tsx`).

**Bug 2 - the `Template` Mongoose schema's own `category` enum never got the lowercase value.** Fixing bug 1 made `normalizeCategory()` emit lowercase `"authentication"` (previously it fell through to the old uppercase `"AUTHENTICATION"`, which the schema already tolerated for synced templates). `Template.create()` with the new lowercase value threw an uncaught `ValidationError`, surfacing to users as a generic `{"error":"SERVER_ERROR","message":"Something went wrong."}` 500 on every attempt to create an Authentication template - reproduced live via the browser's own Network tab. **Fixed**: added lowercase `"authentication"` to `models/Template.js`'s enum alongside the existing uppercase entry.

**Bug 3 - `otpService.js`'s own Mongo queries were broken by this app's `sanitizeFilter: true` setting**, unrelated to anything above but only ever reachable once bugs 1-2 were fixed (every previous OTP attempt failed earlier, before ever reaching this code). Both `generateAndSendOtp()` and `verifyOtp()` queried `VerificationCode` with a raw `consumedAt: { $exists: false }` filter. `server/db.js` globally sets `mongoose.set("sanitizeFilter", true)`, which treats any un-trusted operator object as a literal value to cast against the schema type - `consumedAt` is a `Date`, so every real call threw `Cast to date failed for value "{ '$exists': false }"`. Every other `$exists` usage in this codebase (`apiKeyAuth.js`, `conversations.js`, `dashboard.js`, `assistant.js`, `realtime/events.js`, `aiAssistant.js`) already correctly wraps this in `mongoose.trusted()` - this was an isolated oversight in the OTP flow specifically. **Fixed** by wrapping both filters the same way.

**Bug 4 - the OTP delivery button was sent with the wrong shape, twice, before getting it right.** First attempt guessed `sub_type: "copy_code"` with a `coupon_code`-typed parameter, based on the button's `otp_type: "COPY_CODE"` at *creation* time - Meta rejected the real send with `(#132000) Number of parameters does not match the expected number of params` (a separate, related issue: Meta's create-template response never echoes back its real generated components, so the locally-saved record kept a textless `BODY` component that `buildTemplateComponents()` silently skipped - fixed by re-fetching the template from Meta immediately after a successful Authentication submission). Second attempt, after finding Meta's actual `copy-code-button-authentication-templates` docs, corrected the shape to `sub_type: "url"` / parameter `type: "text"` - but still failed with `(#131008) Required parameter is missing`, because the detection check (`button.type === "OTP"`) only matches the *creation* request shape. **Confirmed by temporarily exposing the raw `components` field via the API** (serializeTemplate never returned it before - now does, permanently, since it's harmless workspace-owned data) that Meta reports the button back as an ordinary `type: "URL"` button whose `url` is Meta's own `whatsapp.com/otp/code/` redirect with a `{{1}}` placeholder in the query string - indistinguishable from a real business URL button (e.g. `jmms_receipt_new`'s genuine "View Receipt" link) by type alone. **Fixed** by also matching that distinctive URL pattern; verified against both templates' real production component data that `signup_verification` now sends correctly and `jmms_receipt_new`'s unrelated URL button is untouched.

**Also renamed the template mid-session**: the first `signup_otp` attempt (rejected under `utility`) was deleted in WhatsApp Manager and recreated - but Meta enforces a short cooldown before reusing a name+language you just deleted, so the new template was named `signup_verification` instead to avoid waiting on that. `WHATSAPP_OTP_TEMPLATE_NAME=signup_verification` was added to the VPS's real `server/.env` to match (the code's own default remains `signup_otp` in `.env.example`, unrelated to this specific instance's actual value).

**Hit the same recurring deploy-cron "stuck partial pull" pattern twice more while pushing these five commits** (`8187428` → `41ba47d` → `272c851` → `ed28473` → `4c99025` → `09d05b2` → `92ebceb`) - both times manually resolved with `git pull` (clean fast-forward each time, never a real conflict) + `pm2 restart dashboard-api` + updating `.last-deploy-sha` by hand so the health-check marker didn't get left stale. See [[dashboard-whatsapp-deploy-cron-broken]] - still not self-healing, still worth a real fix to the cron's locking eventually.

**Real state now**: WhatsApp OTP signup's send path is fully fixed and proven with a real delivered message. The verify step (`POST /whatsapp-otp/verify`) was deliberately left for the user to complete themselves, since it creates a real User account - not something to do unattended even for testing.
